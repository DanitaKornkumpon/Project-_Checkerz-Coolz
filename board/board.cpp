...
// ฟังก์ชันช่วยจัดข้อความให้อยู่กึ่งกลางปุ่มพอดีเป๊ะ
...
int main() {
    ...
    // ==========================================
    // UI สำหรับหน้าเมนู (ดีไซน์ใหม่ ธีมไม้ Pixel Art)
    // ==========================================

    // โหลดรูปภาพพื้นหลังเมนู (ปรับปรุงให้ลองหาหลายๆ ชื่อ)
    sf::Texture menuBgTexture;
    bool hasMenuBg = false;
    std::vector<std::string> bgFileNames = {
        "a08dd583883b147e9996f867f0b4bb7c.jpg",
        "a08dd583883b147e9996f867f0b4bb7c.png",
        "menu_bg.jpg",
        "menu_bg.png"
    };
    ...
    for (const auto& fname : bgFileNames) {
        if (menuBgTexture.loadFromFile(fname)) {
            hasMenuBg = true;
            std::cout << "Successfully loaded background: " << fname << std::endl;
            break; // เจอรูปแล้วหยุดหา
        }
    }

    if (!hasMenuBg) {
        std::cout << "WARNING: Could not load ANY background image!" << std::endl;
        std::cout << "Please place the image in the same folder as your executable (.exe) or project (.vcxproj)" << std::endl;
    }
    std::cout << "--------------------------" << std::endl;
    ...
    if (hasMenuBg) {
        ...
        // ขยายรูปให้เต็มหน้าต่าง 800x800 พอดี
        float scaleX = 800.0f / menuBgTexture.getSize().x;
        float scaleY = 800.0f / menuBgTexture.getSize().y;
        menuBgSprite.setScale(scaleX, scaleY);
    }

    // โหลด Texture ของปุ่ม (ถ้ามีไฟล์รูปภาพที่ตัดมาแล้ว)
    ...
    // สีไม้ (สำหรับกรณีที่ไม่มีไฟล์รูปภาพ จะใช้สีเหล่านี้แทน)
    sf::Color woodNormal(160, 104, 63); // สีไม้อ่อน
    sf::Color woodHover(190, 134, 93);  // สีไม้สว่างเวลาเอาเมาส์ชี้
    sf::Color woodSelected(120, 74, 33); // สีไม้เข้มเวลาถูกเลือก
    sf::Color woodBorder(70, 40, 20);   // สีขอบ

    // 1. หัวข้อเกม
    sf::Text titleText("CHECKERS", font, 80);
    titleText.setFillColor(sf::Color(255, 220, 150)); // สีทองอ่อนๆ
    ...
    if (hasTex1P) btn1P.setTexture(&texBtn1P);
        ...
        btn1P.setFillColor(woodNormal);
        btn1P.setOutlineThickness(4);
        btn1P.setOutlineColor(woodBorder);
    }
    ...
    if (hasTex2P) btn2P.setTexture(&texBtn2P);
        ...
        btn2P.setFillColor(woodNormal);
        btn2P.setOutlineThickness(4);
        btn2P.setOutlineColor(woodBorder);
    }
    ...
    if (hasTexStart) btnStart.setTexture(&texBtnStart);
        ...
        btnStart.setFillColor(sf::Color(100, 180, 80)); // สีเขียวธรรมชาติ
        btnStart.setOutlineThickness(5);
        btnStart.setOutlineColor(woodBorder);
    }
    ...
    if (hasEndgameImage) {
        ...
        endgameSprite.setPosition(0.0f, 0.0f);
    ...
    auto endTurn = [&]() {
        ...
        if (numPlayers == 1 && currentTurn == P2) {
            aiTimer.restart();
        }
    ...
    auto performAIMove = [&]() {
        ...
        for (int idx : p2Pieces) {
            ...
            for (int i = 0; i < dirs; i++) {
                ...
                if (nr >= 0 && nr < BOARD_SIZE && nc >= 0 && nc < BOARD_SIZE) {
                    int nIdx = nr * 10 + nc;
                    if (game.board[nIdx] == EMPTY) {
                        simpleMoves.push_back({ idx, nIdx });
                    }
                }
            }
        }
    ...
    while (window.isOpen()) {
        ...
        // --- ทำระบบ Hover ชี้เมาส์แล้วปุ่มเปลี่ยนสีนิดหน่อย (เพิ่มความสมจริง) ---
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (currentState == MENU) {
                    if (btn1P.getGlobalBounds().contains(mouseX, mouseY)) {
                        numPlayers = 1;
                    }
                    if (btn2P.getGlobalBounds().contains(mouseX, mouseY)) {
                        numPlayers = 2;
                    }
                    ...
                    if (hasEndgameImage && endgameClickArea.contains(mouseX, mouseY)) {
                        clickedToRestart = true;
                    }
                    else if (!hasEndgameImage) {
                        clickedToRestart = true;
                    }
        ...
        if (currentState == PLAYING && numPlayers == 1 && currentTurn == P2) {
            if (aiTimer.getElapsedTime().asSeconds() > 0.6f) {
                performAIMove();
            }
        }

        window.clear(sf::Color(45, 40, 35)); // เปลี่ยนสีพื้นหลังให้ดูเข้ากับปุ่มไม้ (สีน้ำตาลเข้มเทาๆ)

        if (currentState == MENU) {
            // ถ้ารูปภาพโหลดติด ให้วาดรูปลงไปเป็นพื้นหลัง
            if (hasMenuBg) {
                window.draw(menuBgSprite);

                // สร้างแผ่นฟิล์มสีดำแบบโปร่งแสงทับรูปภาพ เพื่อให้ตัวหนังสือ/ปุ่มเด่นขึ้น
                sf::RectangleShape dimOverlay(sf::Vector2f(800.0f, 800.0f));
                dimOverlay.setFillColor(sf::Color(0, 0, 0, 140)); // ยิ่งเลขมาก (เต็มที่ 255) ยิ่งมืด
            ...
            window.draw(btn1P);
            // ถ้าใช้รูปภาพ ไม่ต้องวาดตัวหนังสือทับลงไป
            if (!hasTex1P) window.draw(text1P);

            window.draw(btn2P);
            if (!hasTex2P) window.draw(text2P);

            window.draw(btnStart);
            if (!hasTexStart) window.draw(textStart);
        }
