...
public:
    ...
    void reset() {
        ...
        std::fill(statsBoard.begin(), statsBoard.end(), PieceStats());
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
                ...
                // ตรวจจับการกด Restart เมื่ออยู่หน้าจบเกม
                if (isGameOver) {
                    if (startButtonArea.contains(static_cast<float>(event.mouseButton.x), static_cast<float>(event.mouseButton.y))) {
                        game.reset();
                        isGameOver = false;
                        currentTurn = P1;
                        selectedIndex = -1;
                        availableCaptures.clear();
                        isAIWaiting = false;
                        window.setTitle("Checkers - Red's Turn (Player)");
                    }
                    continue;
                }
